...
### 2026-08-31 - 强化 DAS Ego × UMI 实机研究叙事

本次在 `Selected Work` 最前面的当前设备项目 `DAS Ego + UMI` 上补强研究型叙事：明确 Ego 的第一视角、多相机、姿态与同步职责，UMI 的双手末端、夹持与接触职责，并把“本人已完成的真实桌面实操”和“正在推进的多模态数据整理 / VLA 示教”分层写清。源视频为带 `-90°` 旋转元数据的 4K 竖屏素材；桌面原始文件保持不动，网页使用同分辨率、同帧率的高码率横屏版本，不做 `960×540/1280×720` 降级，同时从同一段实操中提取原始尺寸 PNG 帧。页面不直接放置外部手册直链，避免把官方规格误写成个人实验结论。
...
| 技术叙事 | 依据 GenRobot.AI DAS Ego 与 DAS / UMI 产品资料，归纳 6 路 RGB、约 270° 视场、6 轴 IMU、UMI 触觉/末端状态、MCAP 落盘和 VLA 示教方向；页面不重复放置外部手册直链 |
| 详情页面 | 新增 `RabbitRobot/projects/rabbitrobot-ego-umi.html`，包含横屏视频、官方规格表、四张实操帧、研究定位与诚实状态 |
| 验证结果 | 本地浏览器确认首页与详情页均解码 `3840×2160 / 60fps` 横屏视频；4 行规格、4 张 `3840×2160` 无损 PNG 证据帧均正常；桌面与 `390×844` 手机端无横向溢出；页面直链检查为 0 |
